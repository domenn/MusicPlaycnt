...
msw::tray::Listener::Listener(Tray* tray) : tray_(tray) {
  ...
  change_handle_ = FindFirstChangeNotification(folder_.c_str(), FALSE, FILE_NOTIFY_CHANGE_LAST_WRITE);
...
void msw::tray::Listener::listen() {
  ...
  if (!RegisterWaitForSingleObject(
          &async_change_handle_, change_handle_, &my_wait_or_timer_cb, this, INFINITE, WT_EXECUTEONLYONCE)) {
