...
std::wstring msw::windows::format_windows_error_w(uint32_t error_code, const wchar_t* lpszFunction) {
  LPVOID lpMsgBuf;
  ...
  const auto lpDisplayBuf =
      static_cast<LPVOID>(LocalAlloc(LMEM_ZEROINIT,
                                     (lstrlen(static_cast<LPCTSTR>(lpMsgBuf)) +
                                      static_cast<SIZE_T>(lstrlen(static_cast<LPCTSTR>(lpszFunction))) + 40) *
                                         sizeof(TCHAR)));
