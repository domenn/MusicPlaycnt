...
msw::model::Song::Song(msw_proto_song::Song song) : song_(std::move(song)) {}
msw::model::Song msw::model::Song::deserialize(const std::string& string_contents) {
  msw_proto_song::Song empty_proto_song;

  std::istringstream istringstr(string_contents);
...
void msw::model::Song::testing_create_restore() {
  ...
  Song song2 = Song::deserialize(serd);
