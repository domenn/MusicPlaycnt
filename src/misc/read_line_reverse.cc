...
ReadLineReverse::ReadLineReverse(std::unique_ptr<std::istream> istream)
...
std::string ReadLineReverse::next() {
  ...
  const auto idx_left = idx_unreverse(buffer_,
                                      static_cast<size_t>(reverse_idx_start_of_line_that_is_to_be_read_next_
                                      ));
